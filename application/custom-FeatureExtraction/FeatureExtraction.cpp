...
int main(int argc, char **argv)
{
	...
	// Request for socket connection
	connect(sockfd, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));  // Socket, address pointer, address size
	...
	while (true) // this is not a for loop as we might also be reading from a webcam
	{
		...
		while (!captured_image.empty())
		{
			...
			// auto reg = face_analyser.GetCurrentAUsReg();
			
			// std::string msg("");
			// for (size_t i=0; i<reg.size(); i++) {
			// 	// printf("%lf", reg[i].second);
			// 	// printf("%s.", reg[i].first.c_str());
			// 	msg = msg + std::to_string(reg[i].second).c_str() + ",";
			// }
			// send(sockfd, msg.c_str(), 153, 0);
			...
			if (detection_success && face_model.eye_model) {
				gazeAngle = GazeAnalysis::GetGazeAngle(gazeDirection0, gazeDirection1);
				if (cv::checkRange(gazeAngle)) {
					// std::cout << "Gaze Angle: [" << gazeAngle[0] << ", " << gazeAngle[1] << "]" << std::endl;
				} else {
					gazeAngle[0] = 0.0;
					gazeAngle[1] = 0.0;
				}
			...
			msg = msg + std::to_string(gazeAngle[0]) + "," + std::to_string(gazeAngle[1]) + ",";
